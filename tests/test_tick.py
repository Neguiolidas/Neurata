...
def test_sync_update_in_place_clears_stale_derived_from(tmp_path):
    """Regressão: `_sync_update_in_place` limpa `derived_from` velho ao
    absorver um corpo novo via skill-item — sem isso o arquivo ficaria com
    corpo novo cru + marca de derivação do corpo ANTIGO, e `expand
    --restore` devolveria o full errado (o texto de outro conteúdo).

    Nenhum fluxo automático produz esse estado: `regime_of` chama de
    espelho quem tem `source_key`, e `compact` recusa espelho. Quem chega
    aqui é acervo de disco — o usuário compactou um grão curado e depois
    colou um `source_key` no frontmatter para adotá-lo como espelho de uma
    fonte. O sync não pode confiar na marca herdada desse arquivo."""
    home = _home(tmp_path)
    entry_id = "01MIRRORSYNCFIX000000000"
    # Estado adotado à mão: corpo já compactado + marca do full antigo.
    old_derived_from = archive.put(home, _FULL_BODY.encode("utf-8"))
    body1 = make_summary(_FULL_BODY)
    ...
    new_full_body = (
        ...
        "novo, garantindo que o corpo mudou de verdade.\n"
    )
    # A skill adota o grão: mesmo slug/source_key, corpo novo.
    ...
    assert body2 != body1
    assert meta2.get("derived_from") is None  # marca velha não sobrou
    # O full compactado à mão continua recuperável no archive — o que saiu
    # foi o ponteiro, não o conteúdo.
